...
const InstructorCard: FC<InstructorCardProps> = ({ name, photo, description, socialLinks, className, ...props }) => {
  return (
    <div className={clsx('flex flex-col rounded-[10px] bg-ui-blocks px-9 pb-10 pt-7', className)} {...props}>
      ...
      <Title type="h3" size="h5" className="mb-7 !text-start uppercase text-primary">
        {name}
      </Title>
      ...
      <div className="mt-auto flex gap-5">
        {socialLinks.instagram && (
          <a href={socialLinks.instagram} target="_blank" aria-label="Instagram">
            <InstagramIcon className="transition-all hover:opacity-80" aria-hidden="true" />
          </a>
        )}
        {socialLinks.youtube && (
          <a href={socialLinks.youtube} target="_blank" aria-label="Youtube">
            <YoutubeIcon className="transition-all hover:opacity-80" aria-hidden="true" />
          </a>
        )}
        {socialLinks.pinterest && (
          <a href={socialLinks.pinterest} target="_blank" aria-label="Pinterest">
            <PinterestIcon className="transition-all hover:opacity-80" aria-hidden="true" />
          </a>
        )}
      </div>
    </div>
